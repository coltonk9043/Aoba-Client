...
public class AddonScreen extends Screen {
    ...
    public void init() {
        ...
        this.addDrawableChild(ButtonWidget.builder(Text.of("Open Addons Folder"), button -> Util.getOperatingSystem().open(addonsPath))
        ...
        // Right Side
        this.descriptionWidget = new TextFieldWidget(textRenderer, 282, 112, this.width - 314, this.height - 160, Text.of(""));
    ...
    public void render(DrawContext drawContext, int mouseX, int mouseY, float partialTicks) {
        ...
        if(selectedAddon != null) {
        	// Although selected addon is TECHNICALLY what we need... we need an icon identifier :(
        	AddonSelectionList.NormalEntry entry = (AddonSelectionList.NormalEntry)addonListSelector.getSelectedOrNull();
        	if(entry != null) {
        		drawContext.drawTexture(entry.getIcon(), 282, 68, 0, 0, 48, 48, 48, 48);
        	}
