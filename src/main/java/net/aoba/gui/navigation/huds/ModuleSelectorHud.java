	...
	public void draw(Render2D renderer, DrawContext drawContext, float partialTicks) {
		...
		if (pos.isDrawable()) {
			...
			for (int i = 0; i < categories.size(); i++) {
				renderer.drawString(drawContext, ">>", x + width - 24, y + (ROW_HEIGHT * i) + 8,
						GuiManager.foregroundColor.getValue());

				// Draws the name of the category dependent on whether it is selected.
				if (index == i)
					renderer.drawString(drawContext, "> " + categories.get(i).getName(), x + 8,
							y + (ROW_HEIGHT * i) + 8, GuiManager.foregroundColor.getValue());
				else
					renderer.drawString(drawContext, categories.get(i).getName(), x + 8, y + (ROW_HEIGHT * i) + 8,
							0xFFFFFF);
			}
			...
			if (isCategoryMenuOpen) {
				...
				for (int i = 0; i < modules.size(); i++) {
					if (indexMods == i) {
						renderer.drawString(drawContext, "> " + modules.get(i).getName(), x + width + 5,
