...
public class SettingsWindow extends Window {
	...
	public SettingsWindow() {
		...
		// Title Bar
		stackPanel.addChild(new StringComponent(stackPanel, "Settings Manager"));
		stackPanel.addChild(new SeparatorComponent(stackPanel));

		stackPanel.addChild(new StringComponent(stackPanel, "Profile:", false));

		GridComponent loadConfigGrid = new GridComponent(stackPanel);
		...
		configNames = new ListComponent(loadConfigGrid, SettingManager.configNames);
		configNames.setMargin(new Margin(2f, 4f, 2f, 4f));
		loadConfigGrid.addChild(configNames);

		// Load Config Button
		ButtonComponent btnLoadConfig = new ButtonComponent(loadConfigGrid, new Runnable() {
			@Override
			public void run() {
		...
		StringComponent buttonLoadString = new StringComponent(btnLoadConfig, "Load");
		...
		GridComponent fileSaveGrid = new GridComponent(stackPanel);
		...
		// Config Name TextBox
		fileName = new TextBoxComponent(fileSaveGrid);
		fileName.setMargin(new Margin(8f, 4f, 0f, 4f));
		fileSaveGrid.addChild(fileName);

		// Save Button
		ButtonComponent btnSaveACopy = new ButtonComponent(fileSaveGrid, new Runnable() {
			@Override
			public void run() {
		...
		StringComponent buttonStr = new StringComponent(btnSaveACopy, "Save");
