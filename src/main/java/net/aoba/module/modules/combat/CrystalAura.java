...
public class CrystalAura extends Module implements TickListener {
	...
	public CrystalAura() {
		super(new KeybindSetting("key.crystalaura", "Crystal Aura Key",
				InputUtil.fromKeyCode(GLFW.GLFW_KEY_UNKNOWN, 0)));

		this.setName("CrystalAura");
		this.setCategory(Category.Combat);
		this.setDescription("Attacks anything within your personal space.");

		radius = new FloatSetting("crystalaura_radius", "Radius",
				"Radius, in blocks, that you can place/attack a crystal.", 5f, 1f, 15f, 1f);
		targetFriends = new BooleanSetting("crystalaura_target_friends", "Target Friends", "Target friends.", false);
		attackDelay = new FloatSetting("crystalaura_attack_delay", "Attack Delay",
				"Delay between attacks in milliseconds.", 500, 0, 2000, 50);
		autoSwitch = new BooleanSetting("crystalaura_auto_switch", "Auto Switch",
				"Automatically switch to End Crystal.", true);
		placeDelay = new FloatSetting("crystalaura_place_delay", "Place Delay",
				"Delay between placing crystals in milliseconds.", 500, 0, 2000, 50);
		targetMode = new EnumSetting<>("crystalaura_target_mode", "Target Mode", "Mode to target players.",
				TargetMode.NEAREST);
		multiPlace = new BooleanSetting("crystalaura_multi_place", "MultiPlace",
				"Allows placing multiple crystals simultaneously.", false);
		antiSuicide = new BooleanSetting("crystalaura_anti_suicide", "AntiSuicide",
				"Prevents attacking crystals if it would result in player's death.", true);
		minDamage = new FloatSetting("crystalaura_min_damage", "Min Damage",
				"Minimum damage a crystal must deal to be placed or attacked.", 6f, 0f, 36f, 0.5f);
		maxSelfDamage = new FloatSetting("crystalaura_max_self_damage", "Max Self Damage",
				"Maximum self-damage the player can take from a single crystal.", 4f, 0f, 20f, 0.5f);
		enemyRange = new FloatSetting("crystalaura_enemy_range", "Enemy Range",
				"Maximum distance an enemy can be to be considered a target.", 12f, 0f, 32f, 1f);
		wallRange = new FloatSetting("crystalaura_wall_range", "Wall Range",
				"Distance an enemy must be to a wall for crystals to be placed or attacked through it.", 3f, 0f, 8f,
				0.5f);
		crystalPriority = new EnumSetting<>("crystalaura_crystal_priority", "Crystal Priority",
				"Prioritize which crystals to attack first.", CrystalPriority.CLOSEST);
		rotationMode = new EnumSetting<>("crystalaura_rotation_mode", "Rotation Mode",
				"Controls how the player's view rotates.", RotationMode.NONE);
	...
	private void placeCrystal() {
		...
		for (PlayerEntity player : players) {
			if (player == MC.player || MC.player.squaredDistanceTo(player) > radiusSquared)
				continue;
	...
	private Optional<BlockPos> findBestCrystalPlacement(PlayerEntity player) {
		...
		BlockPos[] positionsToCheck = { playerPos.north(), playerPos.south(), playerPos.east(), playerPos.west(),
				playerPos.north().down(), playerPos.south().down(), playerPos.east().down(), playerPos.west().down() };
