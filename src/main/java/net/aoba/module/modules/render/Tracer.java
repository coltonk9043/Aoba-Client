        ...
        TickListener
{
    ...
    public void onRender(Render3DEvent event)
    {
        ...
        for (Entity entity : sorted)
        {
            ...
            if (entity != null && entity != MC.getCameraEntity() && (MC.getCameraEntity() == null || !entity.equals(MC.getCameraEntity().getRootVehicle())))
            {
                ...
                {
                    bb = new Box(x - 0.25, y + entity.getHeight() - 0.45, z - 0.25, x + 0.25, y + entity.getHeight() + 0.055, z + 0.25);
                }
                else
                {
                ...
                color = new Color(Math.min((int) red, 255), baseColor.getGreen(), baseColor.getBlue(), baseColor.getAlpha());

                final Vec3d rotation = new Vec3d(0, 0, 75).rotateX(-(float) Math.toRadians(renderEntity.getPitch())).rotateY(-(float) Math.toRadians(renderEntity.getYaw())).add(renderEntity.getEyePos());

                Vec3d eyePos = renderEntity.getEyePos();

                if (mode.getValue() == TracerMode.Stem)
                {
                    Render3D.drawLine3D(event.GetMatrix(), new Vec3d(x, y, z), new Vec3d(x, renderEntity.getHeight() + y, z), color, lineWidth.getValue());
                }
                ...
                {
                    case Head ->
                            Render3D.drawLine3D(event.GetMatrix(), start, new Vec3d(x, y + entity.getHeight() - 0.18f, z), color, lineWidth.getValue());
                    case Body ->
                            Render3D.drawLine3D(event.GetMatrix(), start, new Vec3d(x, y + entity.getHeight() / 2.0f, z), color, lineWidth.getValue());
                    case Feet ->
                            Render3D.drawLine3D(event.GetMatrix(), start, new Vec3d(x, y, z), color, lineWidth.getValue());
                }

                if (mode.getValue() == TracerMode.Fill)
                {
                    Render3D.draw3DBox(event.GetMatrix(), bb, new Color(color.getRed(), color.getGreen(), color.getBlue(), 78), 1.0f);
                }
