...
public class NewRender3D implements IRenderer {
    private static final DynamicUniformStorage<UboData> UNIFORM_STORAGE = new DynamicUniformStorage<>("Aoba 3D UBO", 
    ...
    public void render(DrawContext context) {
        if (!triangleBuffer.isEmpty()) {
            renderBuffer(triangleBuffer, AobaRenderPipelines.TRIS, context);
        }
        if (!lineBuffer.isEmpty()) {
            renderBuffer(lineBuffer, AobaRenderPipelines.LINES, context);
        }
    }
    ...
    private void renderBuffer(BufferManager buffer, RenderPipeline pipeline, DrawContext context) {
        ...
        RenderPass pass = RenderSystem.getDevice().createCommandEncoder()
            .createRenderPass(() -> "Aoba 3D Renderer", colorAttachment, OptionalInt.empty());
